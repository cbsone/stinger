...
The `http_plugin` is a core plugin supported by both `stnode` and `keosd`. The plugin is required to enable any RPC API functionality provided by a `stnode` or `keosd` instance.
...
```sh
# command-line
stnode ... --plugin eosio::http_plugin [options]
 (or)
...
Config Options for eosio::http_plugin:
  ...
  --http-server-address arg (=127.0.0.1:8888 for stnode)

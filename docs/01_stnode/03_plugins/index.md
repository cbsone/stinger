...
Plugins extend the core functionality implemented in `stnode`. Some plugins are mandatory, such as `chain_plugin`, `net_plugin`, and `producer_plugin`, which reflect the modular design of `stnode`. The other plugins are optional as they provide nice to have features, but non-essential for the nodes operation.
...
| Plugins add incremental functionality to `stnode`. Unlike runtime plugins, `stnode` plugins are built at compile-time.

...
The plugin-specific options can be configured using either CLI options or a configuration file, `config.ini`. Nodeos-specific options can only be configured from the command line. All CLI options and `config.ini` options can be found by running `stnode --help` as shown above.
...
A custom `config.ini` file can be set by passing the `stnode` option `--config path/to/config.ini`.

## Nodeos Example

The example below shows a typical usage of `stnode` when starting a block producing node:

```sh
stnode --replay-blockchain \
  -e -p eosio \
...
The above `stnode` command starts a producing node by:
...
* redirecting both `stdout` and `stderr` to the `stnode.log` file

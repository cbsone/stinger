...
This section describes how to set up a producing node within the Stinger network. A producing node, as its name implies, is a node that is configured to produce blocks in an `Stinger` based blockchain. This functionality if provided through the `producer_plugin` as well as other [Nodeos Plugins](../../03_plugins/index.md).
...
* `public-key` - A valid Stinger public key in form of a string.
